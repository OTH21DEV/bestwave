...
import { QueryError ,Connection,FieldPacket} from "mysql2";
...
export const getForecast = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const location = await sqlFindForecast(req.query.id as string);
...
async function sqlFindForecast(locationId: string | number): Promise<any[]> {
  const sql = `
    SELECT 
      l.locationID, l.name, l.latitude, l.longitude,
      f.forecastID, f.forecastDate, f.timeSlot,
