...
const getSpotsFromDatabase = async () => {
    return new Promise((resolve, reject) => {
        pool.query("SELECT locationID,name,latitude,longitude FROM locations LIMIT 2", (error, results) => {
            if (error) {
                // reject(error);
...
const calculateDate = (offsetDays, utcHour = 0) => {
    const date = new Date();
...
const filterDataEveryTreeHours = (data) => {
...
const fetchSurfData = async () => {
    try {
        ...
        const requests = spots.map((spot) => axios.get(`https://api.stormglass.io/v2/weather/point`, {
            params: {
                lat: spot.latitude,
                lng: spot.longitude,
        ...
        responses.forEach(async (response, index) => {
            try {
                ...
                if (response && response.data && response.data.hours) {
                    let filteredData = filterDataEveryTreeHours(response.data.hours);
...
//   });
// };
const getLocationIdFromDatabase = async (spotID) => {
    try {
...
//   }
// };
const insertDataIntoDatabase = async (spotID, filteredData) => {
    try {
        const locationID = await getLocationIdFromDatabase(spotID);
        for (const hourData of filteredData) {
            // Parse Date and Time from time field
